...
//			level: "level",			//层级
		...
		this.menubarFun = {
			remove: function(checkbarNodes){			//删除复选框选中节点，需要用户自定义，如未指定则树只是page面上做了修改
				return true;
			}
		};
		this.menubarTips = {
		...
		this.checkbarFun = {
			...
			chooseDone: function(checkbarNodesParam) {	//复选框点击事件完毕后，返回该树关于复选框操作的全部信息，用于用户自定义，如未指定则树只是page面上做了修改
				return ;
			}
		};
		this.iframe = {  // 树点击节点时，打开iframepage面参数配置
	...
	DTree.prototype.loadIframe = function($div, iframeParam) {
		...
		if (flag) {
			if ($(iframeElem).length > 0) {		//iframe存在
				...
				layer.msg("iframe绑定异常，请确认page面中是否有iframepage对应的容器", {icon:5});
				return false;
			}
		}
		return flag;
	...
	var dtree = {
		render: function(options){	// 初始化树
			...
			if(id == "") {
				layer.msg("page面中未找到绑定id", {icon:5});
			} else {
				dTree = DTrees[id];
